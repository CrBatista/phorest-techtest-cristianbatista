...
class JwtAuthenticationFilterTest {
    ...
    void shouldPopulateSecurityContextWhenTokenValid() throws ServletException, IOException {
        ...
        assertThat(SecurityContextHolder.getContext().getAuthentication().getName()).isEqualTo("cristian");
    }

    @Test
    void shouldIgnoreWhenAuthorizationHeaderMissing() throws ServletException, IOException {
        HttpServletRequest request = mock(HttpServletRequest.class);
        HttpServletResponse response = mock(HttpServletResponse.class);
        FilterChain chain = mock(FilterChain.class);
    ...
    void shouldIgnoreWhenTokenInvalid() throws ServletException, IOException {
        ...
        Mockito.when(request.getHeader(HttpHeaders.AUTHORIZATION)).thenReturn("Bearer invalid");
