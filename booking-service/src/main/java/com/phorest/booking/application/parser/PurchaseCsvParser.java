...
@Component
public class PurchaseCsvParser {
    ...
    public PurchaseCsvParser(BookingCsvProperties properties) {
        this.properties = properties;
    }

    public List<PurchaseCsvRecord> parse(InputStream inputStream) {
        ...
        try (Reader reader = new InputStreamReader(inputStream, StandardCharsets.UTF_8);
            ...
            throw new BookingCsvParsingException("Failed to read purchases CSV", ex);
        }
    }
    ...
    private void validateHeaders(CSVParser parser) {
        ...
        if (properties.purchasesHeaders() != null && !properties.purchasesHeaders().isEmpty()) {
            List<String> expected = properties.purchasesHeaders().stream()
    ...
    private PurchaseCsvRecord toRecord(CSVRecord record) {
        String id = record.get("id");
        if (!StringUtils.hasText(id)) {
            throw new BookingCsvParsingException("Purchase id cannot be empty");
        }
        ...
        return new PurchaseCsvRecord(id, appointmentId, name, price, loyaltyPoints);
    }
    ...
    public record PurchaseCsvRecord(String purchaseId,
